...
// specify the colors
  ...
  $pl_name[8] = "Neptün";
  $pl_name[9] = "Pluto";
  $pl_name[10] = "Tepe Noktası";
  $pl_name[11] = "Lilit";		//add a planet
  $pl_name[12] = "Gerçek Düğüm";
  $pl_name[13] = "Şans Noktası";
  $pl_name[14] = "Verteks";
  $pl_name[15] = "Yükselen";
  $pl_name[16] = "Gökyüzü Ortası";
...
// draw in the planet glyphs //tablo burada
  for ($i = 0; $i <= $last_planet_num; $i++)
  {
    ...
    imagettftext($im, 9, 0, ($margins + $left_margin_planet_table + $cell_width * 2) - 25, $cell_height * ($i + 1) - 3, $blue, 'arial.ttf', $pl_name[$i]);
    $sign_num = floor($longitude[$i] / 30) + 1;
    drawboldtext($im, 14, 0, $margins + $left_margin_planet_table + $cell_width * 5 - 10, $cell_height * ($i + 1), $black, 'HamburgSymbols.ttf', chr($sign_glyph[$sign_num]), 0);
    imagettftext($im, 9, 0, $margins + $left_margin_planet_table + $cell_width * 6 - 10, $cell_height * ($i + 1) - 3, $blue, 'arial.ttf', Convert_Longitude($longitude[$i]) . " " . $rx1[$i]);
  }
