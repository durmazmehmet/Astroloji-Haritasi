...
// put planets in chartwheel - person 2
  // sort longitudes in descending order from 360 down to 0
  Sort_planets_by_descending_longitude_synastry($num_planets, $longitude2, $sort2, $sort_pos2);
...
// draw in the aspect lines
  for ($i = 0; $i <= $last_planet_num; $i++)
  {
    for ($j = 0; $j <= $last_planet_num; $j++)
    {
      ...
      if ($sort_pos1[$i] == SE_POF Or $sort_pos2[$j] == SE_POF)
      {
        $orb = 2;
      }
      elseif ($sort_pos1[$i] == SE_LILITH Or $sort_pos2[$j] == SE_LILITH)
      {
        $orb = 3;
      }
      elseif ($sort_pos1[$i] == SE_TNODE Or $sort_pos2[$j] == SE_TNODE)
      {
        $orb = 3;
      }
      elseif ($sort_pos1[$i] == SE_VERTEX Or $sort_pos2[$j] == SE_VERTEX)
      {
        $orb = 3;
      }
      elseif ($sort_pos1[$i] == 0 Or $sort_pos1[$i] == 1 Or $sort_pos2[$j] == 0 Or $sort_pos2[$j] == 1)
      {
        $orb = 8;
      }
      else
...
Function Sort_planets_by_descending_longitude_synastry($num_planets, $longitude, &$sort, &$sort_pos)
{
// load all $longitude[] into sort[] and keep track of the planet numbers in $sort_pos[]
  for ($i = 0; $i <= $num_planets - 1; $i++)
  {
