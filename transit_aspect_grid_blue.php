...
// draw the grid - horizontal lines
  for ($i = 0; $i <= $last_planet_num - 4 + 1; $i++)
  {
    imageline($im, $margins, $cell_height * ($i + 1), $margins + $cell_width * ($num_to_use + 1), $cell_height * ($i + 1), $black);
  }

// draw the grid - vertical lines
  for ($i = 0; $i <= $num_to_use + 1; $i++)
  {
    imageline($im, $margins + $cell_width * $i, $cell_height * ($last_planet_num - 4 + 2), $margins + $cell_width * $i, $cell_height, $black);
  }
...
// draw in the planet glyphs
  for ($i = 0; $i <= $num_to_use; $i++)
  {
    ...
    {
      drawboldtext($im, 16, 0, $margins + $left_margin_planet_table, $cell_height + $cell_height * ($i + 1), $blue, 'HamburgSymbols.ttf', chr($pl_glyph[$i]), 0);
    }
    else
    {
    ...
    if ($i <= $last_planet_num - 4)
    {
      drawboldtext($im, 14, 0, $margins + $left_margin_planet_table + $cell_width * 10, $cell_height + $cell_height * ($i + 1), $blue, 'HamburgSymbols.ttf', chr($sign_glyph[$sign_num2]), 0);
