<?php
  ...
  if (isset($_POST['submitted']) Or isset($_POST['h_sys_submitted']))
  {
    ...
    include ('header_transits_solar_arcs.html');				//here because of setting cookies above
    ...
    if ($start_month != "" And $start_day != "" And $start_year != "")
    {
      ...
      {
        $my_error .= "The transit/solar arc date you entered is not valid.<br />";
      }
    }

    if (($start_year < 1900) Or ($start_year >= 2100))
    {
      $my_error .= "Transit/solar arc date - please enter a year between 1900 and 2099.<br />";
    }
...
//echo $jd_to_use;
      ...
      exec ("swetest -edir$sweph -bj$jd_to_use -ut -p0 -eswe -fl -g, -head", $out);	//add a planet
...
//add a planet - maybe some code needs to be put here
      ...
      {
        $longitude2[$i] = Crunch($longitude1[$i] + $solar_arc);		// for solar arc listing below, not for passing variables to other scripts
      }
...
//display natal data
      ...
      echo "<b>$name2</b><br />";
      echo '<b>On ' . strftime("%A, %B %d, %Y<br />%X (time zone = GMT $tz1 hours)</b><br />\n", mktime($hour1, $minute1, $secs, $start_month, $start_day, $start_year)) . "<br />";

      echo "<b>$name3</b><br />";
      echo '<b>On ' . strftime("%A, %B %d, %Y<br />%X (time zone = GMT)</b><br />\n", mktime($hour3, $minute3, $secs, $start_month, $start_day, $start_year)) . "<br />";
      echo "</font>";
?>
...
<?php
	...
	$longitude2[LAST_PLANET + 2] = $longitude2[LAST_PLANET + 10];
...
// no need to urlencode unless perhaps magic quotes is ON (??)
    ...
    echo "<center>";
    echo "<img border='0' src='transit_sa_wheel.php?p1=$ser_L1&sa=$solar_arc&p3=$ser_L3&hc1=$ser_hc1' width='640' height='640'>";
    echo "<br /><br />";
    echo "SOLAR ARCS<br />";
    echo "<img border='0' src='sa_aspect_grid_for tr_sa.php?rx1=$rx1&rx2=$rx2&p1=$ser_L1&sa=$solar_arc&hc1=$ser_hc1&hc2=$ser_hc2&ubt1=$ubt1&ubt2=$ubt2' width='830' height='475'>";
    echo "<br />TRANSITS<br />";
    echo "<img border='0' src='transit_aspect_grid_blue.php?rx1=$rx1&rx2=$rx3&p1=$ser_L1&p2=$ser_L3&hc1=$ser_hc1&hc2=$ser_hc3&ubt1=$ubt1&ubt2=$ubt3' width='830' height='475'>";
    echo "</center>";
    echo "<br />";
...
// display transit data - aspect table
    ...
    {
      @mail(EMAIL, "Transits and Solar Arcs", "");
    }
...
//display the transit chart report
  ...
  {
    include ('header_transits_solar_arcs.html');				//here because of cookies
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" target="_blank" style="margin: 0px 20px;">
  <fieldset><legend><font size=4><b>Data entry for Transits/Solar Arcs - Enter your birth information</b></font></legend>
